...
/* This the main JS for arcanum in [en] */

function search_memo (){
	
        var q = $("#searchform input[name=q]").val();
        if (q.length < 2){
                $("#searchresult").html("required chars: 2");
                return false;
        }

        $("#searchresult").show().html("searching...");
        ...
        $.ajax({
                ...
                success: function(data) {
                        ...
                        if (data.count == 0){
                                ...
                                $("#searchresult").html("Results: " + data.count);
                        }
...
function load_common_used (id){
	...
	$.get('http://arcanum.dreamwriter.org/portals/common_used_change&id_port_change_common_used=' + id + '&port_change_common_used_old_val=' + oldval, function(data) {
		...
		set_message("Changed favorite setting!");
	...
	$.ajax({  
  		  ...
  		  success: function (reqCode) {
		    		...
		    		if (reqCode != 0) {
						...
						set_message('No portals in this category!');
						$(cat_id).html('');
	    			}
  		 }	   
  		});
...
function do_register (){
	...
	if ($(reg_form + ' #captchatext').val() == '') {
                set_message('Enter captcha text');
                error = true;
	}
    
	if (username == '') {
		set_message('Missing field: Username');
		error = true;
	} else if (password1 == '') {
		set_message('Missing field: Password');
		error = true;
	} else if (password1 != password2) {
		set_message('Passwords are not equal');
		error = true;
	} else if (regexpmatch.test(colour) == false) {
		set_message('Favourite color must be a #HEX_CODE value!');
		error = true;
	}
	...
	if (error == false) { 
		$.ajax({  
			  ...
			  success: function (reqCode) {
					if (reqCode == 1) {
							text = '<b class="info">' + username + '</b>,  your account was created succesfully.';
							$(reg_form).fadeOut(400).delay(400).hide().html(text).fadeIn(400);	
							
					} else if (reqCode == 2){
							reloadcaptcha();
							set_message('captcha value was not correct, a new one was generated');
	        ...
	        $.ajax({  
	    		  ...
	    		  success: function (reqCode) {
			    		if (reqCode == 1) {
	  						...
	  						set_message('Your changes have been saved successfull!');	
	  					} else{
	  						$(cat_form).fadeOut(400).delay(400).hide().html('<div class="error">ERROR [' + reqCode + ']</div>').fadeIn(400);
		    			}
	    		 }
	        ...
	        $.ajax({  
	    		  ...
	    		  success: function (reqCode) {
			    		if (reqCode == 1) {
		    				...
		    				set_message('<div class="error">ERROR [' + reqCode + ']</div>');
			    		}
	    		 }
	    ...
	    function del_cat (id){
	        ...
	        $.ajax({  
	    		  ...
	    		  success: function (reqCode) {
			    		if (reqCode == 1) {
							/*
	  							$(cat_form).fadeOut(400).delay(400).hide().html("deleted").fadeIn(400).delay(400).fadeOut(400);
	  							$(cat_form).hide();
	  						*/
	  						$(cat_form).fadeOut(400);
	  						set_message("deleted");
	  						...
	  						set_message('<div class="error"">There are files in this category!</div>');
		    			} else {
		    				set_message('<div class="error">ERROR [' + reqCode + ']</div>');
			    		}
	    		 }
	    ...
	    function del_portal (id){
	        ...
	        $.ajax({  
	    		  ...
	    		  success: function (reqCode) {
			    		if (reqCode == 1) {
	  						/*
	  						$(portal_form).fadeOut(400).delay(400).hide().html("deleted").fadeIn(400).delay(400).fadeOut(400);
	  						*/
	    	...
	    	function doarclogin () {
				...
				$.post('http://arcanum.dreamwriter.org/login', { username: encodeURIComponent(username), password: encodeURIComponent(password), patternlock: patternlock },
		   			...
		   			function(data) { 
		   				...
		   				if (ret.code == '0'){
		   					$('#login_text').html('Login successful.');
...
function timer_regenerate () {
	$.get('http://arcanum.dreamwriter.org/timer/regenerate', 
		function(data) {
				...
				window.location = 'http://arcanum.dreamwriter.org/login/logout&msg=s:103:"Owing to am internal problem, you have been logged out. Arcanums admin was informed about this problem.";';
                        }
		}
	);
}
